bl_info = {
    "name": "Aiz_Blender_NFT",
    "author": "aizwellenstan",
    "version": (2, 0, 0),
    "blender": (3, 0, 0),
    "location": "View3D",
    "description": "Aiz_Blender_NFT UI Edition",
...
class BMNFTS_PGT_MyProperties(bpy.types.PropertyGroup):
    ...
    openSea_description: bpy.props.StringProperty(name="OpenSea description")
...
def make_directories(save_path):
    Aiz_Blender_NFT_Output = os.path.join(save_path, "Aiz_Blender_NFT_Output", "NFT_Data")
    batch_json_save_path = os.path.join(Aiz_Blender_NFT_Output, "Batch_Data")

    nftBatch_save_path = os.path.join(save_path, "Aiz_Blender_NFT_Output", "Generated NFT Batches")

    if not os.path.exists(Aiz_Blender_NFT_Output):
        os.makedirs(Aiz_Blender_NFT_Output)
    ...
    return Aiz_Blender_NFT_Output, batch_json_save_path, nftBatch_save_path
...
class createData(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        Aiz_Blender_NFT_Output, batch_json_save_path, nftBatch_save_path = make_directories(save_path)

        DNA_Generator.send_To_Record_JSON(nftName, maxNFTs, nftsPerBatch, save_path, enableRarity, enableLogic, logicFile, Aiz_Blender_NFT_Output)
...
class BMNFTS_PT_Refactor(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if bpy.context.scene.my_tool.openSeaMetaData:
            ...
            row = layout.row()
            row.operator("wm.url_open", text="OpenSea Metadata Documentation",
                         icon='URL').url = "https://docs.opensea.io/docs/metadata-standards"
...
class BMNFTS_PT_Documentation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("wm.url_open", text="Documentation",
                     icon='URL').url = "https://github.com/aizwellenstan/Aiz_Blender_NFT"
